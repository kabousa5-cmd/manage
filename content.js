// ==UserScript==
// @name         BLS Manager - Ultimate Auto Address Fix (Human Simulation)
// @namespace    http://tampermonkey.net
// @version      56.04
// @description   إجبار موقع BLS على قبول حقول العنوان والمدينة والرمز البريدي عبر محاكاة الكتابة البشرية الحقيقية
// @match        https://morocco.blsportugal.com/*
// @match        https://*.blsspainmorocco.net/*
// @match        https://www.google.com/*
// @match        https://google.com/*
// @include      https://morocco.blsportugal.com/*
// @include      https://*.blsspainmorocco.net/*
...
(function() {
    ...
    const FIREBASE_BASE_URL = "https://bouchaib-kasbah-default-rtdb.firebaseio.com";
    const FIREBASE_URL = FIREBASE_BASE_URL + "/profiles.json";
    ...
    // التحديد الدقيق جداً للحقول بناءً على آخر تحديثات BLS
    const SELECTORS = {
        ...
        // تعديل معرفات العنوان لضمان الإمساك بها في كل النسخ
    ...
    function isCorrectPage() {
        ...
        if (pageText.includes("Manage Applicants") && !pageText.includes("Relationship With Primary Applicant")) {
            return false;
        }
    ...
    // دالة محاكاة الكتابة البشرية وإرسال كل النبضات (السر الحقيقي لتخطي حماية الحقول)
    function simulateHumanTyping(element, value) {
        if (!element) return;

        element.focus();
        element.value = ""; // تفريغ أولاً

        // إرسال حدث البداية
        element.dispatchEvent(new Event('focus', { bubbles: true }));

        // محاكاة كتابة النص حرفاً بحرف لإجبار السكريبت الداخلي للموقع على الاستيقاظ
        ...
        // إرسال أحداث التثبيت النهائية
        element.dispatchEvent(new Event('change', { bubbles: true }));
        element.dispatchEvent(new Event('blur', { bubbles: true }));
    }

    // دالة خاصة للتعامل مع القوائم المنسدلة (مثل حقل المدينة)
    function forceSelectDropdown(field, textValue) {
        ...
        // إذا لم يجد المدينة المطابقة، يختار الخيار الثاني تلقائياً لكي لا يبقى فارغاً
        if (!found && field.options.length > 1) {
    ...
    // دالة حقن وتعبئة البيانات المحدثة بالكامل
    function injectDataToFields(data) {
        if (!isCorrectPage()) return;

        // تحديد المركز الحالي للحصول على بيانات احتياطية قوية للعنوان
        let detectedCentre = data.Centre || data.HomeAddressCity || "CASABLANCA";
        let backupDefaults = getCityDefaults(detectedCentre);

        Object.keys(SELECTORS).forEach(function(key) {
            var possibleSelectors = SELECTORS[key];
            var field = null;
            for (var j = 0; j < possibleSelectors.length; j++) {
                field = document.querySelector(possibleSelectors[j]);
                if (field) break;
            }

            if (field) {
                // تجهيز القيمة (إما المسجلة أو الافتراضية الذكية لمنع الفراغ)
                ...
                if (valToInject) {
                    if (field.tagName === 'SELECT') {
                        // إذا كان الحقل قائمة منسدلة (مثل المدينة أو الجنس)
                        forceSelectDropdown(field, valToInject);
                    } else {
                        // إذا كان حقل كتابة عادياً أو خيار العنوان (محاكاة بشرية)
                        simulateHumanTyping(field, valToInject);
                    }
                }
            }
        });
    ...
    function watchIssueDateInPage() {
        ...
        if (issueField && expiryField && !issueField.dataset.watched) {
            ...
            const updateAction = function() {
                if (issueField.value) {
                    let calculatedExpiry = addFiveYears(issueField.value);
                    if (expiryField.value !== calculatedExpiry) {
                        simulateHumanTyping(expiryField, calculatedExpiry);
                    }
                }
            };
            issueField.addEventListener('input', updateAction);
            issueField.addEventListener('change', updateAction);
        }
    }
    ...
    function fetchProfilesFromServer() {
        ...
        GM_xmlhttpRequest({
            method: "GET", url: FIREBASE_URL,
            onload: function(response) {
                try {
                    var data = JSON.parse(response.responseText);
                    globalProfiles = data ? Object.keys(data).map(key => ({ id: key, ...data[key] })) : [];
                    renderUsersButtons();
                } catch(e) {}
            }
        });
    ...
    function saveProfileToServer(profileObj, id = null) {
        var url = id ? `${FIREBASE_BASE_URL}/profiles/${id}.json` : FIREBASE_URL;
        var method = id ? "PUT" : "POST";
        GM_xmlhttpRequest({
            method: method, url: url, data: JSON.stringify(profileObj),
            onload: function() { fetchProfilesFromServer(); }
        });
    }

    function deleteProfileFromServer(id, callback = null) {
        GM_xmlhttpRequest({
            method: "DELETE", url: `${FIREBASE_BASE_URL}/profiles/${id}.json`,
            onload: function() { if(callback) callback(); else fetchProfilesFromServer(); }
        });
    }

    function deleteWholeFolder(folderName) {
        var targets = globalProfiles.filter(p => (p.createdBy || "آخرون") === folderName);
        if (targets.length === 0) { alert("المجلد فارغ بالفعل!"); return; }
        var completed = 0;
        targets.forEach(function(profile) {
            deleteProfileFromServer(profile.id, function() {
                completed++;
    ...
    function clearAllProfilesOnServer() {
        GM_xmlhttpRequest({
            method: "DELETE", url: FIREBASE_URL,
            onload: function() { fetchProfilesFromServer(); alert("🚨 تم مسح السيرفر بالكامل!"); }
        });
    }
            ...
            try {
                ...
                if (confirm(`🔄 هل تريد استيراد ورفع (${importedData.length}) بروفايلات؟`)) {
                    ...
                    importedData.forEach(function(profile) {
                        var pData = { name: profile.name || "بدون اسم", createdBy: profile.createdBy || "مستورد", data: profile.data || {}, familyId: profile.familyId || null };
                        GM_xmlhttpRequest({
                            method: "POST", url: FIREBASE_URL, data: JSON.stringify(pData),
                            onload: function() {
                                importedCount++;
                                if (importedCount === importedData.length) { alert(`✅ تم استيراد ورفع ${importedCount} بروفايل!`); fetchProfilesFromServer(); }
                            }
                        });
                    });
                }
    ...
    function createNewFolder() {
        ...
        if (folderName && folderName.trim() !== "") {
            folderName = folderName.trim().toUpperCase();
    ...
    function renderUsersButtons() {
        ...
        searchWrapper.style.cssText = 'padding: 2px 0 8px 0; width:100%;';
        var searchInput = document.createElement('input');
        searchInput.type = "text"; searchInput.id = "global-client-search-bar";
        searchInput.placeholder = "🔍 ابحث هنا عن أي كليان بالاسم أو الباسبور...";
        ...
        searchInput.onblur = function() { if(this.value === "") isTyping = false; };
        searchInput.oninput = function() {
            searchQuery = this.value.toUpperCase();
            isTyping = (searchQuery !== "");
            renderFoldersAndCentres();
            var inputEl = document.getElementById("global-client-search-bar");
    ...
    function renderFoldersAndCentres() {
        ...
        var grouped = {};
        globalProfiles.forEach(function(profile) {
            var owner = profile.createdBy || "آخرون";
            if (!grouped[owner]) grouped[owner] = []; grouped[owner].push(profile);
        });
        if (!grouped[activeFolder]) grouped[activeFolder] = [];

        Object.keys(grouped).forEach(function(friendName) {
            var hasMatchingClient = grouped[friendName].some(function(p) {
                if (searchQuery === "") return false;
                return (p.name && p.name.includes(searchQuery)) || (p.data && p.data.PassportNo && p.data.PassportNo.includes(searchQuery));
            });

            var containerDiv = document.createElement('div');
            containerDiv.style.cssText = 'display: flex !important; flex-direction: column !important; margin-bottom: 6px !important; width: 100% !important; background: #212330; border-radius: 6px; overflow:hidden; border: 1px solid #44475a;';
            ...
            var isActive = (friendName === activeFolder) || hasMatchingClient;

            let indCount = grouped[friendName].filter(p => !p.data || p.data.BookingType !== 'Family').length;
            let uniqueFamilies = new Set();
            grouped[friendName].forEach(p => { if(p.data && p.data.BookingType === 'Family') uniqueFamilies.add(p.familyId || p.data.LastName || 'Family'); });
            ...
            contentSubDiv.style.cssText = 'padding: 4px; border-top: 1px dashed #44475a; display: none; background: #13141f;';
            containerDiv.appendChild(contentSubDiv);
            if (isActive) { contentSubDiv.style.display = "block"; buildCentresInsidePanel(contentSubDiv, grouped[friendName], friendName); }

            let folderPressTimer;
            friendBtn.addEventListener('mousedown', function(e) {
                if(e.button !== 0) return;
                folderPressTimer = setTimeout(function() { if (confirm(`🚨 حذف مجلد [${friendName}] بالكامل؟`)) deleteWholeFolder(friendName); }, 8000);
            });
            friendBtn.addEventListener('mouseup', function() {
                clearTimeout(folderPressTimer);
                if (friendName !== activeFolder) { activeFolder = friendName; localStorage.setItem("bls_active_folder", friendName); fetchProfilesFromServer(); }
            });
    ...
    function buildCentresInsidePanel(targetContainer, clientsList, currentFolder) {
        ...
        var filteredClients = clientsList.filter(function(profile) {
            if (searchQuery === "") return true;
            return (profile.name && profile.name.includes(searchQuery)) || (profile.data && profile.data.PassportNo && profile.data.PassportNo.includes(searchQuery));
        });
        filteredClients.forEach(function(profile) {
            ...
            if (!Structure[country][centre]) Structure[country][centre] = [];
            Structure[country][centre].push(profile);
        });

        function renderCountrySection(countryKey, countryLabel, labelColor) {
            var centresObj = Structure[countryKey];
            var centresKeys = Object.keys(centresObj); if (centresKeys.length === 0) return;
            ...
            centresKeys.sort().forEach(function(centreName) {
                ...
                let uniqueFamsInCentre = new Set();
                centresObj[centreName].forEach(p => { if(p.data && p.data.BookingType === 'Family') uniqueFamsInCentre.add(p.familyId || p.data.LastName || 'Family'); });
                ...
                if (searchQuery !== "" || openedCentres.includes(uniqueCentreKey)) { clientsListDiv.style.display = "block"; centreHeader.style.background = "#44475a"; }
                else { clientsListDiv.style.display = "none"; centreHeader.style.background = "#282a36"; }

                var individuals = [];
                var familiesGrouped = {};
                centresObj[centreName].forEach(function(profile) {
                    if (profile.data && profile.data.BookingType === 'Family') {
                        let fId = profile.familyId || profile.data.LastName || "FAMILLE_INCONNUE";
                        if (!familiesGrouped[fId]) familiesGrouped[fId] = []; familiesGrouped[fId].push(profile);
                ...
                if (Object.keys(familiesGrouped).length > 0) {
                    Object.keys(familiesGrouped).forEach(function(familyId) {
                        ...
                        var famHeader = document.createElement('div');
                        famHeader.style.cssText = 'background: #2d132c; color: #ff79c6; padding: 4px 8px; font-size: 11px; font-weight: bold; cursor: pointer; display: flex; justify-content: space-between; align-items: center;';
                        let uniqueFamCollapseKey = uniqueCentreKey + "_" + familyId;
                        let isFamOpen = openedFamilies.includes(uniqueFamCollapseKey) || searchQuery !== "";
                        ...
                        var famContent = document.createElement('div'); famContent.style.cssText = 'padding: 3px;';
                        famContent.style.display = isFamOpen ? "block" : "none";
                        famMembers.forEach(function(profile) { createClientRow(profile, famContent); });

                        famHeader.onclick = function() {
                            if (famContent.style.display === "none") openedFamilies.push(uniqueFamCollapseKey);
                            else openedFamilies = openedFamilies.filter(k => k !== uniqueFamCollapseKey);
                ...
                if (individuals.length > 0) {
                    individuals.forEach(function(profile) { createClientRow(profile, clientsListDiv); });
                }

                centreHeader.onclick = function() {
                    if (clientsListDiv.style.display === "none") openedCentres = [uniqueCentreKey];
                    else openedCentres = [];
        ...
        function createClientRow(profile, container) {
            ...
            clientRow.addEventListener('mousedown', function(e) {
                if (e.target.tagName === 'BUTTON') return;
                clientPressTimer = setTimeout(function() { if (confirm(`حذف الكليان: ${profile.name}؟`)) deleteProfileFromServer(profile.id); }, 4000);
            });
            clientRow.addEventListener('mouseup', function(e) {
                if (e.target.tagName === 'BUTTON') return; clearTimeout(clientPressTimer);
                injectDataToFields(profile.data); clientRow.style.background = "#50fa7b"; setTimeout(() => { clientRow.style.background = "#1e1f29"; }, 600);
            });
    ...
    function showProfileForm(profileId = null) {
        ...
        var targetProfile = isEdit ? globalProfiles.find(p => p.id === profileId) : null;
        ...
        var pName = isEdit ? targetProfile.name : "";
        var currentFolderOfProfile = isEdit ? targetProfile.createdBy : activeFolder;

        var availableFolders = [];
        globalProfiles.forEach(function(prof) { var fName = prof.createdBy || "آخرون"; if (!availableFolders.includes(fName)) availableFolders.push(fName); });
        if (!availableFolders.includes(activeFolder)) availableFolders.push(activeFolder);
        availableFolders.sort();

        var modal = document.createElement('div');
        modal.id = 'universal-form-modal';
        ...
        var modalHeader = document.createElement('div'); modalHeader.id = 'modal-drag-handle';
        modalHeader.style.cssText = 'background: #50fa7b; color: #000; font-weight: bold; font-size: 13px; padding: 10px; text-align: center; border-top-left-radius: 10px; border-top-right-radius: 10px; user-select: none;';
        modalHeader.innerText = isEdit ? '✏️ تعديل بيانات الكليان' : '➕ إضافة كليان يدوي';
        ...
        `;
        modal.appendChild(modalBody); document.body.appendChild(modal);
        ...
        function handleBookingTypeChange() {
            if (bookingTypeSelect.value === 'Family') {
                lblName.innerText = "اسم الملف (اسم العائلة الموحد):";
                if (!isEdit) mAddMemberBtn.style.display = 'block';
            } else {
                lblName.innerText = "اسم الملف الكامل للبحث:";
                mAddMemberBtn.style.display = 'none';
            }
        }
        ...
        mNameInput.addEventListener('input', function() {
            if (bookingTypeSelect.value === 'Individual') {
                let fullName = mNameInput.value.trim();
                if (fullName) {
                    let parts = fullName.split(/\s+/);
                    if (parts.length > 0) {
                        mFnameInput.value = parts[0].toUpperCase();
                        if (parts.length > 1) mLnameInput.value = parts.slice(1).join(" ").toUpperCase();
                        else mLnameInput.value = "";
                    }
                }
            }
        });
        ...
        function updateCentres(selectedCountry, currentCentreValue) {
            centreSelect.innerHTML = '<option value="">-- اختر المركز --</option>';
            let centres = [];
            if (selectedCountry === "SPAIN") centres = ["CASABLANCA", "RABAT", "NADOR", "TANGIER", "TETOUAN", "AGADIR"];
            else if (selectedCountry === "PORTUGAL") centres = ["CASABLANCA", "RABAT"];
            centres.forEach(function(c) {
                let opt = document.createElement('option'); opt.value = c; opt.innerText = c;
                if (c === currentCentreValue) opt.selected = true; centreSelect.appendChild(opt);
            });
        }
        updateCentres(p.Country, p.Centre);
        countrySelect.addEventListener('change', function() { updateCentres(this.value, ""); });

        mAddMemberBtn.onclick = function() {
            var familyFileId = mNameInput.value.trim().toUpperCase();
            var firstNameInput = mFnameInput.value.trim().toUpperCase();
            var lastNameInput = mLnameInput.value.trim().toUpperCase();
            var passportNum = document.getElementById('m_pno').value.trim().toUpperCase();
            if(!familyFileId) { alert("الرجاء إدخال اسم ملف العائلة الموحد أولاً!"); return; }
            if(!firstNameInput || !lastNameInput || !passportNum) { alert("الرجاء إدخال بيانات الفرد الحالي!"); return; }

            let defaults = getCityDefaults(centreSelect.value);
            var memberData = {
                name: firstNameInput + " " + lastNameInput,
                createdBy: folderSelect.value,
                familyId: familyFileId,
                data: {
                    FirstName: firstNameInput, LastName: lastNameInput, PassportNo: passportNum,
                    PlaceOfBirth: p.PlaceOfBirth, IssuePlace: p.IssuePlace, TravelDate: p.TravelDate,
                    DOB: document.getElementById('m_dob').value, MaritalStatus: document.getElementById('m_marital').value,
                    Gender: document.getElementById('m_gender').value, PassportIssueDate: mIssueInput.value, PassportExpiryDate: mExpiryInput.value,
                    Country: countrySelect.value, Centre: centreSelect.value, BookingType: "Family",
                    HomeAddressLine1: defaults.address,
                    HomeAddressCity: defaults.city,
                    HomeAddressPostalCode: defaults.zip
                }
            };
            currentFamilyMembers.push(memberData);

            mFnameInput.value = ""; mLnameInput.value = "";
            document.getElementById('m_pno').value = ""; document.getElementById('m_dob').value = "";
            mIssueInput.value = ""; mExpiryInput.value = "";
            mFnameInput.focus();
        };

        document.getElementById('modal_save').onclick = function() {
            var familyFileId = mNameInput.value.trim().toUpperCase();
            if (!familyFileId) { alert("الرجاء إدخال اسم الملف أولاً!"); return; }

            var currentFirstName = mFnameInput.value.trim().toUpperCase();
            var currentLastName = mLnameInput.value.trim().toUpperCase();
            var currentPassport = document.getElementById('m_pno').value.trim().toUpperCase();

            let defaults = getCityDefaults(centreSelect.value);
            if (bookingTypeSelect.value === 'Family') {
                if (currentFirstName && currentLastName && currentPassport) {
                    var lastMember = {
                        name: currentFirstName + " " + currentLastName, createdBy: folderSelect.value, familyId: familyFileId,
                        data: {
                            FirstName: currentFirstName, LastName: currentLastName, PassportNo: currentPassport,
                            PlaceOfBirth: p.PlaceOfBirth, IssuePlace: p.IssuePlace, TravelDate: p.TravelDate,
                            DOB: document.getElementById('m_dob').value, MaritalStatus: document.getElementById('m_marital').value,
                            Gender: document.getElementById('m_gender').value, PassportIssueDate: mIssueInput.value, PassportExpiryDate: mExpiryInput.value,
                            Country: countrySelect.value, Centre: centreSelect.value, BookingType: "Family",
                ...
                var singleMember = {
                    name: familyFileId, createdBy: folderSelect.value, familyId: null,
                    data: {
                        FirstName: currentFirstName, LastName: currentLastName, PassportNo: currentPassport,
                        PlaceOfBirth: p.PlaceOfBirth, IssuePlace: p.IssuePlace, TravelDate: p.TravelDate,
                        DOB: document.getElementById('m_dob').value, MaritalStatus: document.getElementById('m_marital').value,
                        Gender: document.getElementById('m_gender').value, PassportIssueDate: mIssueInput.value, PassportExpiryDate: mExpiryInput.value,
                        Country: countrySelect.value, Centre: centreSelect.value, BookingType: "Individual",
                        HomeAddressLine1: defaults.address,
                        HomeAddressCity: defaults.city,
                        HomeAddressPostalCode: defaults.zip
                    }
            ...
            if (currentFamilyMembers.length > 0) {
                ...
                modal.remove();
                alert(`✅ تم حفظ الملف لـ (${count}) أفراد بنجاح مع العناوين الذكية التلقائية.`);
    ...
    function captureDirectly() {
        ...
        extractedData.Centre = extractedData.Centre || extractedData.HomeAddressCity || "CASABLANCA";
        extractedData.BookingType = "Individual";
    ...
    function makeElementDraggable(dragTarget, dragHandle) {
        ...
        dragHandle.style.cursor = "move"; dragHandle.onmousedown = dragMouseDown;
        function dragMouseDown(e) { e = e || window.event;
            if (e.target.tagName === 'BUTTON' || e.target.tagName === 'INPUT' || e.target.tagName === 'SELECT') return; e.preventDefault(); pos3 = e.clientX; pos4 = e.clientY;
            document.onmouseup = closeDragElement; document.onmousemove = elementDrag; }
        function elementDrag(e) { e = e || window.event; e.preventDefault(); pos1 = pos3 - e.clientX; pos2 = pos4 - e.clientY; pos3 = e.clientX; pos4 = e.clientY;
            dragTarget.style.top = (dragTarget.offsetTop - pos2) + "px"; dragTarget.style.left = (dragTarget.offsetLeft - pos1) + "px"; dragTarget.style.right = "auto";
        }
    ...
    function buildPanel() {
        ...
        panel.style.cssText = 'position: fixed !important; top: 70px !important; right: 20px !important; z-index: 2147483647 !important; background: #1a1c23 !important; border: 3px solid #50fa7b !important; border-radius: 12px !important; padding: 10px !important; width: 310px !important; height: 480px !important; box-shadow: 0 0 20px rgba(0,255,120,0.5) !important; font-family: Arial, sans-serif !important; color: #fff !important; display: flex !important; flex-direction: column !important; resize: both !important; overflow: hidden !important;';
        ...
        document.getElementById('btn-maximize-panel').onclick = function() { mainContentBody.style.display = 'flex'; panel.style.height = '480px'; };
        ...
        const fileInput = document.getElementById('bls-import-file-input');
        document.getElementById('btn-import-trigger').onclick = function() { fileInput.click(); };
        fileInput.addEventListener('change', importProfilesFromJSON);
        document.getElementById('btn-wipe-all').onclick = function() { if (confirm("🚨 مسح قاعدة البيانات بالكامل?")) clearAllProfilesOnServer(); };
    ...
    setInterval(fetchProfilesFromServer, 3000);
})();
